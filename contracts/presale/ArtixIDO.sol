...
contract ArtixIDO is Ownable {
    ...
    mapping(address => bool) public boughtARTIX;
    ...
    constructor(
        ...
        require(_maiArtixLP != address(0));

        ARTIX = _ARTIX;
        MAI = _MAI;
        treasury = _treasury;
        maiArtixLP = _maiArtixLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function purchaseARTIX(uint256 _amountMAI) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtARTIX[msg.sender] == false, 'Already participated');

        boughtARTIX[msg.sender] = true;
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all artixs to be sold');

        uint256 maiInTreasure = 250000 * 1e18;

        IERC20(MAI).approve(treasury, maiInTreasure);
        uint256 artixMinted = ITreasury(treasury).deposit(maiInTreasure, MAI, 0);

        require(artixMinted == 250000 * 1e9);

        // dev: create lp with 15 MAI per ARTIX
        IERC20(MAI).transfer(maiArtixLP, 750000 * 1e18);
        IERC20(ARTIX).transfer(maiArtixLP, 50000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(maiArtixLP).mint(address(this));
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(
            maiArtixLP,
            lpBalance
        );

        IUniswapV2Pair(maiArtixLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(
            lpBalance,
            maiArtixLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any ARTIX');
        IERC20(ARTIX).approve(staking, artixMinted);
