...
contract ArtixStakingRev1 is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        uint _gons = IsARTIX( sARTIX ).gonsForBalance( _amount );
        ...
        IERC20( sARTIX ).safeTransfer( _recipient, _amount );
     ...
     */
       function canClaim ( address _recipient ) public view returns ( uint ) {
        ...
        if (info.expiry <= epoch.number && info.expiry != 0) {
            _canClaim = IsARTIX(sARTIX).balanceForGons(info.gons);
        }
        return _canClaim;
    }
    ...
    function settleWarmupInfo(address _user, uint _amount) external {
        require(msg.sender == sARTIX, 'access deny');
        Claim storage claim = warmupInfo[ _user ];
        uint _unstakeGons = IsARTIX( sARTIX ).gonsForBalance( _amount );
    ...
    */
    function changeWarmupInfo(address _user, uint _amount) external {
        ...
        if(tempClaim.expiry != 0) {
            Claim storage claim = warmupInfo[ _user ];
            claim.gons = claim.gons.add( IsARTIX( sARTIX ).gonsForBalance( _amount ));
        } else {
            warmupInfo[ _user ] = Claim ({
                gons: tempClaim.gons.add( IsARTIX( sARTIX ).gonsForBalance( _amount ) ),
                expiry: epoch.number,
                stakeEpochNumber: epoch.number
            });
        }
