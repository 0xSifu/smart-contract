...
contract ArtixTreasuryRev1 is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sARTIXQueue; // Delays change to sARTIX address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sARTIX).balanceOf(msg.sender); // Can only borrow against sARTIX held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithARTIX(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(ARTIX).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SARTIX) {
      // 9
