...
contract ArtixStakingDistributor is Ownable {
    ...
    constructor( address _treasury, address _artix, uint _epochLength, uint _nextEpochTime ) {
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochTime <= block.timestamp ) {
            ...
            for ( uint i = 0; i < info.length; i++ ) {
                if ( info[ i ].rate > 0 ) {
                    IArtixTreasury( treasury ).mintRewards( // mint and send from treasury
                        info[ i ].recipient,
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( ARTIX ).totalSupply().mul( _rate ).div( 1000000 );
    }
