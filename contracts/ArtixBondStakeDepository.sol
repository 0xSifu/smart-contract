...
contract ArtixBondStakeDepository is Ownable {
    ...
    address public immutable ARTIX; // intermediate reward token from treasury
    ...
    struct Bond {
        uint256 payout; //artix at the moment of bond
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        IArtixBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
        ...
        returns (uint256 pendingPayout_)
    {
        ...
        uint256 payout = IsARTIX(sARTIX).balanceForGons(
