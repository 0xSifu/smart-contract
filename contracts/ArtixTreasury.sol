...
contract ArtixTreasury is Ownable, IArtixTreasury {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SARTIX }
    ...
    address public sARTIX;
    uint public sARTIXQueue; // Delays change to sARTIX address
    ...
    constructor (
        ...
        isLiquidityToken[ _ARTIXDAI ] = true;
        liquidityTokens.push( _ARTIXDAI );
        bondCalculator[_ARTIXDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for ARTIX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external override returns ( uint send_ ) {
        ...
        // mint ARTIX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ARTIX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ARTIX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external override {
        ...
        uint value = valueOfToken( _token, _amount );
        IARTIXERC20( ARTIX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sARTIX ).balanceOf( msg.sender ); // Can only borrow against sARTIX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ARTIX
        @param _amount uint
     */
    function repayDebtWithARTIX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IARTIXERC20( ARTIX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ARTIX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external override {
        ...
        IERC20Mintable( ARTIX ).mint( _recipient, _amount );
    ...
    function excessReserves() public override view returns ( uint ) {
        return totalReserves.sub( IERC20( ARTIX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ARTIX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view override returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ARTIX decimals
            value_ = _amount.mul( 10 ** IERC20( ARTIX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SARTIX ) { // 9
            sARTIXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
