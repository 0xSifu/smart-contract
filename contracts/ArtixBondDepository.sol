...
contract ArtixBondDepository is Ownable {
    ...
    struct Bond {
        uint payout; // ARTIX remaining to be paid
    ...
    function deposit(
        ...
        uint value = IArtixTreasury( treasury ).valueOfToken( principle, _amount );
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) ARTIX
         */
        IERC20( principle ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( principle ).approve( address( treasury ), _amount );
        IArtixTreasury( treasury ).deposit( _amount, principle, profit );

        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( ARTIX ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( ARTIX ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( ARTIX ).approve( stakingHelper, _amount );
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IArtixBondingCalculator( bondCalculator ).markdown( principle ) ).div( 100 );
        } else {
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( ARTIX ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if ( isLiquidityBond ) {
            return debtRatio().mul( IArtixBondingCalculator( bondCalculator ).markdown( principle ) ).div( 1e9 );
        } else {
            return debtRatio();
        }
    }
